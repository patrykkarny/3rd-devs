...
async function main3() {
  try {
    ...
    console.log(
      "Vector indexes 'actor_index' and 'movie_index' are online and ready.",
    );
    ...
    for (const role of actedIn) {
      const actor = await neo4jService.findNodeByProperty(
        'Actor',
        'name',
        role.actor,
      );
      const movie = await neo4jService.findNodeByProperty(
        'Movie',
        'title',
        role.movie,
      );
      if (actor && movie) {
        await neo4jService.connectNodes(actor.id, movie.id, 'ACTED_IN', {
    ...
    for (const director of directors) {
      const directorNode = await neo4jService.addNode('Director', {
        name: director.name,
      });
    ...
    const movieResults = await neo4jService.performVectorSearch(
      'movie_index',
      searchQuery,
      1,
    );
    const actorResults = await neo4jService.performVectorSearch(
      'actor_index',
      searchQuery,
      1,
    );
    ...
    result1.records.forEach((record) => {
      console.log(
        `${record.get('actor.name')} as ${record.get('role.character')}`,
      );
    });
    ...
    result2.records.forEach((record) => {
      console.log(
        `${record.get('movie.title')} (${record.get(
          'movie.releaseYear',
        )}) as ${record.get('role.character')}`,
      );
    });
    ...
    result3.records.forEach((record) => {
      console.log(
        `${record.get('director.name')} directed: ${record
          .get('movies')
          .join(', ')}`,
      );
    });
    ...
    result4.records.forEach((record) => {
      console.log(
        `${record.get('movie.title')}: ${record.get('actorCount')} actors`,
      );
    });
    ...
    result5.records.forEach((record) => {
      console.log(
        `${record.get('actor1.name')} and ${record.get(
          'actor2.name',
        )} worked together in: ${record.get('movies').join(', ')}`,
      );
    });
    ...
    result6.records.forEach((record) => {
      console.log(
        `${record.get('movie.title')}: ${record.get(
          'oldestActor.name',
        )} (born ${record.get('oldestActor.birthYear')})`,
      );
    });
  } catch (error) {
